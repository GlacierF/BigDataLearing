...
//            此处主要是对数据的长度做判断，因为是要计算均值，所以这个值的（一条
//            一条读进来）
...
//        soi   <count,sum>  soi.count  获得值，soi.sum
        //定义中间结果的输入
        ...
        public ObjectInspector init(Mode mode, ObjectInspector[] parameters)
            ...
            if (mode == Mode.PARTIAL1 || mode == Mode.PARTIAL2) {
                ...
                // part1 阶段1 ObjectInspectorFactory.getStandardStructObjectInspector(fname, foi);(sum:100,count:10)
                // part2 阶段2 ObjectInspectorFactory.getStandardStructObjectInspector(fname, foi);(sum:300,count:40)
        ...
        public void iterate(AggregationBuffer agg, Object[] parameters)
            ...
            //agg可能是一个中间结果     agg.sum =2000  agg.count=20
            //parameters=100
            assert (parameters.length == 1);
            Object p = parameters[0];
            if (p != null) {
                //  myagg.sum =2000  myagg.count=20
                AverageAgg myagg = (AverageAgg) agg;
                try {
